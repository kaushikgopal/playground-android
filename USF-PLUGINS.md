...
This is an advanced USF feature. Read @USF.md first for core USF concepts.
...
class ProductListViewModel(
    ...
    init {
        ...
        register(
            ...
            // Transform plugin effects to parent events
...
### Effect-to-Event Adapter (transformEffect)

Advanced feature allowing plugins to trigger parent ViewModel events through their effects:
...
class PaginationPlugin<T> : UsfPlugin<Event, State<T>, Effect>() {
    ...
    override suspend fun ResultScope<State<T>, Effect>.process(event: Event) {
        when (event) {
            is Event.LoadMore -> {
...
For comprehensive testing patterns and examples, see @USF-TESTING.md.
...
For more patterns, see @USF.md. For testing plugins, see @USF-TESTING.md.
