...
class SimpleEffectToEventTest {
  ...
  fun testEffectToEventAdapter() = runTest {
    ...
    val adapter =
        UsfEffectToEventAdapter<TestPluginEffect, TestEvent> { pluginEffect ->
          when (pluginEffect) {
  ...
  fun testPluginRegistrarWithEffectToEvent() = runTest {
    ...
    val registrar =
        UsfPluginRegistrarImpl<TestEvent, TestState, TestEffect>(
            state = state,
    ...
    registrar.register(
        ...
        mapEffect = null,
        transformEffect =
            UsfEffectToEventAdapter { pluginEffect ->
              when (pluginEffect) {
                is TestPluginEffect.TriggerParentReset -> TestEvent.PluginTriggeredReset
                else -> null
              }
